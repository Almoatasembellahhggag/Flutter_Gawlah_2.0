...
class HomePlaceViewModel extends BaseModel {
  final NavigationService _navigationService = locator<NavigationService>();
  final FirestoreService _firestoreService = locator<FirestoreService>();
  final DialogService _dialogService = locator<DialogService>();

  List<Place> _posts;
  List<Place> get posts => _posts;

  void listenToPosts() {
    setBusy(true);

    _firestoreService.listenToPostsRealTimePlaces().listen((postsData) {
      
      List<Place> updatedPosts = postsData;
      if (updatedPosts != null && updatedPosts.length > 0) {
        _posts = updatedPosts;
        notifyListeners();
      }
  ...
  Future deletePost(int index) async {
    ...
    if (dialogResponse.confirmed) {
      setBusy(true);
      await _firestoreService.deletePostPlace(_posts[index].documentId);
      setBusy(false);
    }
  }

  Future navigateToCreateView() async {
    await _navigationService.navigateTo(CreatePlaceViewRoute);
  }

  void editPost(int index) {
    
    _navigationService.navigateTo(CreatePlaceViewRoute,
